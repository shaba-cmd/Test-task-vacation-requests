...
export function validateCreate(body) {
  ...
  else if (employeeName.length > 200) errors.employeeName = 'ФИО не длиннее 200 символов';
  ...
  else if (reason.length > MAX_TEXT) errors.reason = `Причина не длиннее ${MAX_TEXT} символов`;
...
export function validateReject(body) {
  ...
  const errors = {};
  if (!rejectionReason) errors.rejectionReason = 'Причина отклонения обязательна';
